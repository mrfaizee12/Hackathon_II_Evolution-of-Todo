...
from src.database.config import settings  # Absolute import
from src.database.database import get_session  # Absolute import
...
from src.tools.user_tools import get_user_identity  # Absolute import
from src.services.mcp_tool_server import mcp_server  # Absolute import
...
class OpenRouterService:
    ...
    def _execute_tool(self, function_name: str, arguments: dict, user_id: str, session: Any) -> dict:
        """
        Execute the appropriate tool function based on the function name.
        This method implements silent argument injection by forcing user_id and session into the call.
        """
        # Force String ID: Convert the user_id (UUID) to a string immediately
        user_id_str = str(user_id)

        # UUID Conversion: If the database functions strictly require a UUID object instead of a string
        from uuid import UUID
        actual_id = UUID(user_id_str) if isinstance(user_id_str, str) and len(user_id_str) > 10 else user_id_str

        # Explicit Injection: Before calling any function, inject user_id and db session
        arguments['user_id'] = actual_id  # Using the converted UUID if needed
        arguments['db'] = session  # Using db as the database session parameter
        ...
        try:
            if function_name == "add_task":
                ...
                result = mcp_server.add_task_tool(input_data, actual_id, session)
            elif function_name == "list_tasks" or function_name == "get_all_tasks":
                # Both list_tasks and get_all_tasks should call the get_all_tasks function
                result = get_all_tasks(user_id=actual_id, db_session=session)
            elif function_name == "complete_task":
                input_data = CompleteTaskInput(**arguments)
                result = mcp_server.complete_task_tool(input_data, actual_id, session)
            elif function_name == "delete_task":
                input_data = DeleteTaskInput(**arguments)
                result = mcp_server.delete_task_tool(input_data, actual_id, session)
                ...
                if 'completed' in arguments and not remaining_keys:
                    ...
                    from src.models.todo import Todo  # Absolute import
                    from sqlmodel import select

                    # Get the existing task to fill in missing required fields
                    existing_task_stmt = select(Todo).where(Todo.id == arguments['task_id'], Todo.user_id == actual_id)
                    existing_task = session.exec(existing_task_stmt).first()
                    ...
                    result = mcp_server.update_task_tool(input_data, actual_id, session)
            elif function_name == "set_task_recurrence":
                input_data = SetRecurrenceInput(**arguments)
                result = set_task_recurrence(input_data, actual_id, session)
            elif function_name == "remove_task_recurrence":
                input_data = RemoveRecurrenceInput(**arguments)
                result = remove_task_recurrence(input_data, actual_id, session)
            elif function_name == "set_task_due_date":
                input_data = SetDueDateInput(**arguments)
                result = set_task_due_date(input_data, actual_id, session)
            elif function_name == "set_task_reminder":
                input_data = SetReminderInput(**arguments)
                result = set_task_reminder(input_data, actual_id, session)
            elif function_name == "list_upcoming_tasks":
                input_data = ListUpcomingTasksInput(**arguments)
                result = list_upcoming_tasks(input_data, actual_id, session)
            elif function_name == "get_user_identity":
                result = get_user_identity(actual_id, session)
            else:
                result = {
                    "success": False,
            ...
            # Handle Empty Tool Result: Log the EXACT error message to the console
        ...
        # Ensure tool results are not None and have a "success" key
        if result is None:
    ...
    def chat_completion_v2(self, messages: List[Dict], tools: Optional[List[Dict]] = None,
        ...
        try:
            ...
            # Get user identity for the system message
            user_info = get_user_identity(user_id, None) if user_id else {"name": "User"}
            current_user_name = user_info.get("name", "User")

            # System Message Update: Add this at the very top of your messages list
            system_message = {
                "role": "system",
                "content": f"User Identity: Name is {current_user_name}, ID is {user_id}. You must use this ID for all tools. Never ask the user for their ID. When a user asks to update or delete a task by its number (e.g., 'Task 5'), first use get_all_tasks to find the UUID, then IMMEDIATELY call update_task or delete_task with that UUID. Do not wait for further user confirmation."
            }
            ...
            from ..database.database import get_session  # Absolute import
            from uuid import UUID
            db_gen = get_session()
            session = next(db_gen)  # Get the session
            ...
            try:
                ...
                # Force Multi-step Execution: Process tool calls in a loop to handle multiple rounds
                # Cast user_id to UUID object for database compatibility
                user_uuid = str(UUID(user_id)) if user_id and isinstance(user_id, str) and len(user_id) > 10 else user_id

                # Process tool calls in a loop to handle multiple rounds of execution
                ...
                while iteration < max_iterations:
                    ...
                    for tool_call in tool_calls:
                        ...
                        try:
                            ...
                            if function_name in ["update_task", "delete_task", "complete_task", "set_task_recurrence", "remove_task_recurrence", "set_task_due_date", "set_task_reminder"] and 'task_id' in arguments:
                                ...
                                if task_id_str.isdigit():
                                    ...
                                    if contextual_task_id:
                                        ...
                                        get_all_result = get_all_tasks(user_uuid, session)
                                        if get_all_result.get("success") and "tasks" in get_all_result:
                                            ...
                                            retry_result = get_all_tasks(user_uuid, session)
                            ...
                            # Execute tool logic using the helper method
                            result = self._execute_tool(function_name, arguments, user_uuid, session)

                            # Handle Empty Tool Result: If a tool returns an error, log the EXACT error message to the console
                            if not result.get("success"):
                                print(f"Tool execution error for {function_name}: {result.get('message', 'Unknown error')}")

                            # Ensure tool results are not None and handle session memory updates
                            if result is None:
                                result = {"success": True, "message": "Operation completed successfully", "data": []}

                            # If a tool mutation occurred, update the cached task list
                            if function_name in ["add_task", "update_task", "delete_task", "complete_task", "set_task_recurrence", "remove_task_recurrence", "set_task_due_date", "set_task_reminder"] and result.get("success"):
                                updated_tasks_result = get_all_tasks(user_uuid, session)
                                if updated_tasks_result.get("success") and "tasks" in updated_tasks_result:
                                    self.session_memory[str(user_uuid)] = {
                                        'cached_tasks': updated_tasks_result["tasks"],
                                        'timestamp': time.time()
                                    }
                                    # Append the updated list as a separate tool result for AI context
                                    tool_results.append({
                                        "tool_call_id": f"refresh_{tool_call.id}",  # Unique ID for refresh
                                        "function_name": "get_all_tasks",
                                        "result": {"success": True, "message": "Updated task list after mutation.", "tasks": updated_tasks_result["tasks"]}
                                    })
                            ...
                            error_result = {"success": False, "message": f"I couldn't process that request: {str(tool_error)}", "data": []}
                            tool_results.append({"tool_call_id": tool_call.id, "function_name": function_name, "result": error_result})

                    # Add the tool responses to the message history so the AI can see the results
                ...
                # At this point, we have the final response after all tool executions
                final_response = response
                        ...
                        if success_operations:
                            ...
                            for op in success_operations:
                                ...
                                if func_name == "add_task" and result_data.get("success"):
                                    final_output = f"✅ Okay {current_user_name}, I've added '{result_data.get('task_title', 'your task')}' to your list!"
                                    ...
                                    if updated_tasks_result.get("success") and "tasks" in updated_tasks_result:
                                        ...
                                        if task_list:
                                            ...
                                            final_output = f"✏️ Okay {current_user_name}, I've updated task #{task_number} for you!"
                                            ...
                                            final_output = f"✏️ Okay {current_user_name}, I've updated the task for you! Your task list is now empty."
                                    else:
                                        final_output = f"✏️ Okay {current_user_name}, I've updated the task for you!"
                                    specific_response_found = True
                                    break
                                    ...
                                    if updated_tasks_result.get("success") and "tasks" in updated_tasks_result:
                                        ...
                                        if task_list:
                                            task_list_text = "\\n".join([f"{i+1}) {task.get('title', 'Untitled')} — {'Completed' if task.get('completed') else 'Pending'}" for i, task in enumerate(task_list)])
                                            final_output = f"🗑️ Okay {current_user_name}, task has been removed from your list!\\n\\nHere's your updated task list:\\n{task_list_text}"
                                    ...
                                    if updated_tasks_result.get("success") and "tasks" in updated_tasks_result:
                                        ...
                                        if task_list:
                                            task_list_text = "\\n".join([f"{i+1}) {task.get('title', 'Untitled')} — {'Completed' if task.get('completed') else 'Pending'}" for i, task in enumerate(task_list)])
                                            final_output = f"✅ Okay {current_user_name}, task has been {status_text}!\\n\\nHere's your updated task list:\\n{task_list_text}"
                                    ...
                                    if task_list:
                                        task_list_text = "\\n".join([f"{i+1}) {task.get('title', 'Untitled')} — {'Completed' if task.get('completed') else 'Pending'}" for i, task in enumerate(task_list)])
                                        final_output = f"Okay {current_user_name}, you have {len(task_list)} tasks:\\n{task_list_text}"
                                        ...
                                        final_output = f"Okay {current_user_name}, you don't have any tasks right now."
                                    specific_response_found = True
                                    break
                                    ...
                                    if task_list:
                                        task_list_text = "\\n".join([f"{i+1}) {task.get('title', 'Untitled')} — {'Completed' if task.get('completed') else 'Pending'}" for i, task in enumerate(task_list)])
                                        final_output = f"Okay {current_user_name}, here are your {len(task_list)} tasks:\\n{task_list_text}"
                            ...
                            if not specific_response_found:
                                ...
                                final_output = f"Okay {current_user_name}, I've handled your request. Let me know if you need anything else!"
                        else:
                            ...
                            if failed_ops:
                                ...
                                if failed_func_name == "update_task" and ("missing" in error_msg.lower() or "required" in error_msg.lower() or "title" in error_msg.lower()):
                                    ...
                                    final_output = f"Okay {current_user_name}, I need more information to update the task. Could you please provide the missing details like title, description, or other fields you'd like to update for task {failed_task_id}?"
                                # Retry mechanism: if parsing failed, suggest user rephrase or try again
                                    ...
                                    current_tasks_result = get_all_tasks(user_uuid, session)
                                    ...
                                    final_output = f"Okay {current_user_name}, I'm having trouble with that request: {error_msg}\\n\\nPlease try rephrasing your request or check your task numbers."
                            else:
                                final_output = f"Okay {current_user_name}, I've handled your request. Let me know if you need anything else!"
                ...
                if not response_text or response_text.strip() == "" or response_text.strip() == "I processed your request.":
                        response_text = f"Okay {current_user_name}, I've handled your request. Let me know if you need anything else!"
                ...
                try:
                    if 'db_gen' in locals() and db_gen is not None:
                        next(db_gen)
